    ...
    <div>
       <section class="bg-gray-50 min-h-screen flex items-center justify-center">
        ...
        <div class="md:w-1/2 px-8 md:px-16">
          <h2 class="font-bold text-2xl mb-8 text-[#7f5539]">Forgot Password</h2>

            <form action="" class="flex flex-col">
              <div className='mb-4'>
                <input class="p-2 w-full  bg-gray-200 mt-1 rounded  border" type="email" name='email' value={credetials?.email}
                                        onChange={(e) => {
                                            updateData(e);
                                        }} placeholder="Email" required/>
                                        {errMsg && !credetials?.email && <div className="text-danger text-start mt-2">Email is required!</div>}
                                        </div>  
                                         <button class="bg-[#7f5539]  rounded mr-10 text-white py-2 w-28  hover:scale-105 duration-300" onClick={(e) => loginHandler(e)}>Get otp</button>
            </form>
        </div>
