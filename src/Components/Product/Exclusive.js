    ...
    if(userData){
        ...
        axios.post(baseUrl + "product/get-product",body,config).then((res)=>{
            ...
            data = res?.data?.product.filter((v)=>{
                if(v?.category?._id === "6418b716ca7fde5a4e8cb552"){
                    return v
                }
            })
            setData(data)
...
<div className='py-4'> 
                        ...
                        <div className='space-y-2 space-x-2'>
                            <div className='flex items-center'>
                                <input type="checkbox" id="cat-1" className='ml-2 text-primary focus:ring-0 rounded-sm cursor-pointer' checked={filterData?.letter === "Wooden Stool"} onClick={()=>setFilterData({...filterData,letter:"Wooden Stool"})}/>  
                                <label for="cat-1" className='text-gray-600 ml-3 cursol-pointer'>  Wooden Stool</label>
                            
                            </div>

                            <div className='flex items-center'>
                                <input type="checkbox" id="cat-1" className='text-primary focus:ring-0 rounded-sm cursor-pointer'checked={filterData?.letter === "Temple"} onClick={()=>setFilterData({...filterData,letter:"Temple"})}/>  
                                <label for="cat-1" className='text-gray-600 ml-3 cursol-pointer '>Temple</label>
                                
                            </div>

                            <div className='flex items-center'>
                                <input type="checkbox" id="cat-1" className='text-primary focus:ring-0 rounded-sm cursor-pointer'checked={filterData?.letter === "Drawer's Chest"} onClick={()=>setFilterData({...filterData,letter:"Drawer's Chest"})}/>  
                                <label for="cat-1" className='text-gray-600 ml-3 cursol-pointer'>Drawer Chest</label>
